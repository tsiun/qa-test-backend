...
class TestGetStatBusinessLogic:
    def test_average_calculation(
        ...
        grade_1 = grade_factory(teacher_id=teacher.id, student_id=student.id, grade=2)
        grade_2 = grade_factory(teacher_id=teacher.id, student_id=student.id, grade=3)
        grade_3 = grade_factory(teacher_id=teacher.id, student_id=student.id, grade=3)
    ...
    def test_count_avg_grade_in_empty_group(
        ...
        assert stats.avg is None, (
            f"Wrong calculating, Actual: '{stats.avg}', but expected: 'None'"
        )
